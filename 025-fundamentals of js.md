...
위에서 함수 밖에서 name을 호출하는 것은 당연히 에러를 발생시킬 것이다.  
...
### 클로저 장점 1. 은닉화
ES6에서 class 문법이 생기면서 private을 사용할 수 있지만, prototype처럼 JS는 여전히 프로토타입 기반의 언어이며 내부 동작 방식이 같지 않다.  
...
```javascript
function hello(name) {
  const _name = name // 없이 아래 return 함수 내에 바로 name을 전달해줘도 클로저임
